    ...
    async parseCustomItems() {
        ...
        if (directResult && typeof directResult === 'object' && directResult.type) {
            ...
            if (directResult.config) {
                try {
                    ...
                    console.warn('Failed to apply config overrides from direct input:', error?.message || error);
                }
            }
                    ...
                    if (decodedResult && typeof decodedResult === 'object' && decodedResult.type) {
                        if (decodedResult.config) {
                            try {
                                ...
                                console.warn('Failed to apply config overrides from decoded input:', error?.message || error);
                            }
                        }
        ...
        for (const url of urls) {
            ...
            try {
                ...
                console.warn(`Skipping invalid subscription line: ${url}`, error);
                continue;
            }
            ...
            for (const processedUrl of processedUrls) {
                                ...
                                for (const item of result) {
                                    if (item && typeof item === 'object' && item.tag) {
                                        ...
                                        try {
                                            ...
                                            console.warn(`Skipping invalid nested proxy: ${item}`, error);
                                        }
                                    }
                                }
                            }
                ...
                try {
                    const result = await ProxyParser.parse(trimmedUrl, this.userAgent);
                    ...
                    if (result && typeof result === 'object' && (result.type === 'yamlConfig' || result.type === 'singboxConfig' || result.type === 'surgeConfig')) {
                        if (result.config) {
                            try {
                                ...
                                console.warn('Failed to apply config overrides from parsed input:', error?.message || error);
                            }
                        }
                        if (Array.isArray(result.proxies)) {
                            result.proxies.forEach(proxy => {
                    ...
                    console.warn(`Skipping invalid proxy: ${trimmedUrl}`, error);
                    continue;
                }
            }
        }
        ...
        for (const item of validItems) {
            if (item?.tag) {
                try {
                    ...
                    console.warn(`Skipping invalid custom item: ${item?.tag || '[no tag]'}`, error);
                }
            }
        }
    }
